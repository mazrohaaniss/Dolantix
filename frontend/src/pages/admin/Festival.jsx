...
const Festival = () => {
  ...
  const fetchEvents = async () => {
    try {
      const res = await axios.get('/api/events/category/festival', {
  ...
  const handleAddEvent = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('/api/events', { ...newEvent, category: 'festival' }, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div style={{ display: 'flex' }}>
      ...
      <div style={{ marginLeft: '220px', padding: '20px', width: '100%' }}>
        <h1>Manajemen Acara Festival</h1>
        <button onClick={handleLogout} style={{ float: 'right' }}>Logout</button>

        <h2>Tambah Acara Festival</h2>
        ...
        <h2>Daftar Acara Festival dan Tiket</h2>
        {events.map(event => (
